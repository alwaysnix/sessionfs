...
Extends the Spike 1A parser to produce validated .sfs output directories.

Usage:
    # Convert a specific session by UUID
    python src/spec/convert_cc.py <session-uuid> --output /path/to/output/

    # Convert a specific JSONL file
    python src/spec/convert_cc.py /path/to/session.jsonl --output /path/to/output/

    # Convert and validate
    python src/spec/convert_cc.py <session-uuid> --output /path/to/output/ --validate

    # List available sessions
    python src/spec/convert_cc.py --list
"""
...
# Add spikes directory to path for the Spike 1A parser
SPIKES_DIR = Path(__file__).parent.parent / "spikes"
sys.path.insert(0, str(SPIKES_DIR))

from spike_1a_cc_read import (
    discover_sessions,
    find_session_path,
    parse_session,
    ParsedSession,
...
def _convert_content_block(block: CCContentBlock) -> dict[str, Any]:
    """Convert a Spike 1A ContentBlock to an .sfs content block."""
...
def _convert_message(msg: CCMessage) -> dict[str, Any]:
    """Convert a Spike 1A Message to an .sfs message entry."""
    # Map CC roles to .sfs roles
    role_map = {
        "user": "user",
        "assistant": "assistant",
        "summary": "system",
    ...
    if role == "system" and msg.role == "summary":
        # Convert summary messages to a summary content block
        summary_text = ""
        ...
        # For user messages with tool results, split into tool role messages
        # during the main conversion pass. Here, convert blocks directly.
    ...
    entry: dict[str, Any] = {
        "msg_id": msg.uuid or str(uuid.uuid4()),
...
def extract_file_refs(messages: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """Extract file references from tool_use and tool_result blocks."""
    file_roles: dict[str, str] = {}  # path -> role (highest precedence wins)
    role_priority = {"deleted": 5, "created": 4, "edited": 3, "written": 2, "read": 1, "referenced": 0}
    ...
    for msg in messages:
        for block in msg.get("content", []):
            ...
            # Skip absolute paths — only track workspace-relative
            if file_path.startswith("/"):
                continue

            # Determine role from tool name
            tool_role_map = {
                "Read": "read",
                "Write": "written",
                "Edit": "edited",
...
def convert_session(
    ...
    Args:
        cc_session: Parsed session from Spike 1A.
        output_dir: Parent directory for the output .sfs session.
    ...
    # Fix dangling parent_msg_id references: CC tree structure may reference
    # messages that were filtered (summary, progress, system types). Set
    # dangling references to null to maintain referential integrity.
