...
835 tests passing. Eight-tool capture + four-tool resume (auto-launch). Autosync (off/all/selective modes, debounce, watchlist, dashboard settings). Local storage management (pruning, retention, disk warnings). Multi-provider email (Resend, SMTP, none). Team handoff with email + session copy on claim. LLM-as-a-Judge (BYOK, OpenRouter, consensus, custom base URL, model discovery). Shared project context (CLI + API + MCP tool). MCP server (local + remote, 5 tools). Full-text search. GitHub PR App. Admin dashboard. Helm chart (EKS validated, single-ingress via nginx). Tier-based sync limits (50MB free, 300MB paid).
