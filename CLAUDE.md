...
819 tests passing. Eight-tool capture + four-tool resume (auto-launch). Local storage management (pruning, retention, disk warnings). Multi-provider email (Resend, SMTP, none). Team handoff with email + workspace resolution. LLM-as-a-Judge (BYOK, OpenRouter, consensus, export, stored keys, trust indicators, custom base URL for LiteLLM/vLLM/Ollama). Shared project context (CLI + API + MCP tool). MCP server (local + remote, 5 tools). Full-text search. GitHub PR App. Admin dashboard. Helm chart (EKS validated). Tier-based sync limits (50MB free, 300MB paid).
