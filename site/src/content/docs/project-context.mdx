...
The SessionFS MCP server exposes 54 tools, including several for project context, knowledge, rules, personas (Pro+), tickets (Team+), CI agent runs (Team+), and handoffs (Pro+ for direct, Team+ for team handoffs). When connected, AI agents can read the shared document, contribute knowledge entries, update wiki pages, fetch canonical rules, manage personas, run a ticket through its full FSM, record AgentRun outcomes, and create + claim coordination handoffs with provenance carry-through. This works with any MCP-aware tool (Claude Code, Cursor, etc.).
