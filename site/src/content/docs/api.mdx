...
Available scopes: `sessions:read`, `handoffs:read`, `handoffs:write`, `tickets:read`, `tickets:write`, `personas:read`, `personas:write`, `knowledge:read`, `knowledge:write`, `rules:read`, `rules:write`, `agent_runs:read`, `agent_runs:write`, `retrieval_audit:read`, `admin:*`. The `*` wildcard is reserved for legacy user/admin keys; service keys must enumerate explicitly.
